(function ($) {
    ...
    $.fn.circliful = function (options, callback) {
        ...
        function animate(current) {
            ...
            }else if( curPerc - endPercent > 1 ){
                curPerc -= curStep;
                requestAnimationFrame(function () {
                    animate(Math.max(curPerc, endPercent) / 100);
                }, obj);
    ...
    return function(total, cur, postfix){

        endPercent = parseInt(cur*100/total);
        console.log('endPercent', endPercent);
        if( $text )
            $text.text( cur+' / '+total+' '+postfix );
        if( Math.abs(endPercent-curPerc) > 1  )
            animate(curPerc);
